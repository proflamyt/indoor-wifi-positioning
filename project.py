...
def get_aps(platform, interface='wlan0'):
    ...
    if platform == 'linux' or platform == 'linux2':
        scan_cmd  = subprocess.Popen(['iwlist', interface, 'scan'],  stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        scan_out, scan_err = scan_cmd.communicate()
        scan_out_data = {}
        ...
        for each_line in scan_out_lines:
            if 'Address' in each_line:
                trigger += 1
                scan_out_data[f'Address{trigger}'] = {"ap_mac" : each_line.split(': ')[1].strip()}

            if 'Signal' in each_line:
                level = each_line.split("level=")[1].strip()
                scan_out_data[f'Address{trigger}'] = {"RSSI": int(level.split(" ")[0])}
